...
from Altron import OWNER_ID
from Altron.events import register
from Altron import telethn
...
async def lego(event):
    ...
    try:
        ...
        fillcolor = "black"
        shadowcolor = "blue"
        fnt = glob.glob("./Altron/resources/fonts/*")
        ...
        await telethn.send_file(
            event.chat_id, file=fname, caption=f"ʟᴏɢᴏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ ᴀʟᴛʀᴏɴ ✘"
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception as e:
        await event.reply(f"ғʟᴏᴏᴅᴡᴀɪᴛ ᴇʀʀᴏʀ, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ @Altron_X")


__mod_name__ = "Lᴏɢᴏ​"

__help__ = """
Altron Robot can create some beautiful and attractive logo for your profile pics.
