...
import Altron.modules.sql.global_bans_sql as sql
from Altron.modules.sql.users_sql import get_user_com_chats
from Altron import (
    DEV_USERS,
    EVENT_LOGS,
    OWNER_ID,
...
from Altron.modules.helper_funcs.extraction import (
    extract_user,
    extract_user_and_text,
)
from Altron.modules.helper_funcs.misc import send_to_list
...
def gban(update: Update, context: CallbackContext):
    ...
    if EVENT_LOGS:
        ...
        send_to_list(bot, PYTHON + DEMONS, log_message, html=True)
        ...
        try:
            ...
            if excp.message in GBAN_ERRORS:
                ...
                if EVENT_LOGS:
                    ...
                    send_to_list(
                        bot, PYTHON + DEMONS, f"Could not gban due to: {excp.message}"
                    )
...
def ungban(update: Update, context: CallbackContext):
    ...
    if EVENT_LOGS:
        ...
        send_to_list(bot, PYTHON + DEMONS, "un-gban complete!")
...
def __user_info__(user_id):
    ...
    if user_id == dispatcher.bot.id:
        return ""
    if int(user_id) in PYTHON + TIGERS + WOLVES:
        return ""
    if is_gbanned:
